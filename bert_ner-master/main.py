...
def train_epoch(model, iterator, optimizer, criterion, tokenizer):
    ...
    for i, batch in enumerate(iterator):
        ...
        ce_loss = 0.5 * (criterion(logits, input_tags) + criterion(logits, input_tags))

        kl_loss = compute_kl_loss(logits, logits2)

        # carefully choose hyper-parameters
        loss = ce_loss + 0.6 * kl_loss
...
def evaluate(model, iterator, f, ner_label, verbose = False, predict = False):
...
def predict(hp):
    if(hp.out_name is not None):
        ...
        if (hp.checkpoint is not None):
            ...
            model.load_state_dict(torch.load(hp.checkpoint))
            ...
            fname = os.path.join(hp.logdir, 'dict.pt')
            ner_label.save(fname)
            out_file = hp.out_name
            precision, recall, f1 = evaluate(model, test_iter, out_file, ner_label, verbose=False,predict=True)
        else:
            print("Please enter the checkpoint PATH!! hint: --chekpoint ")
    else:
        print("Please enter the output file name!! hint: --out_name")
...
if __name__=="__main__":
    ...
    parser.add_argument("--bert_model_dir", type=str, default='../LM_pretrained_thuner',
    ...
    parser.add_argument("--logdir", type=str, default="checkpoints/finetuning/lab106")
    parser.add_argument("--dataset", type=str, default="lm_raw_data_thuner")
    parser.add_argument("--trainset", type=str, default="../data/dataset_thuner/ner/train_thuner.txt", help="../data/dataset_finance/MSRA_train_dev_title.char.bmes")
    parser.add_argument("--validset", type=str, default="../data/dataset_thuner/ner/dev_thuner.txt", help="../data/dataset_finance/annual_report_anno_part1&2_title.txt")
    parser.add_argument("--testset", type=str, default="../data/dataset_thuner/ner/test_thuner.txt", help="")
    parser.add_argument("--decodeset", type=str, default="../data/dataset_finance/annual_report_anno_part1&2_title.txt", help="")
    parser.add_argument("--out_name", type=str,help="")
